...
import models.config as configs
...
class Part_Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        for i in range(1, length):
            last_map = torch.matmul(self.sigma * x[i], last_map)



        softmax = torch.nn.Softmax(dim=2)
        soft_map = softmax(last_map[:, :, 1:, 1:])  # 8 12 784 784

        temp_map = soft_map.clone()
        log_map = temp_map.log()

        ent_map = soft_map * log_map
        ent_map = torch.sum(ent_map, dim=2)

        _, max_inx = ent_map.min(2)
        return _, max_inx
